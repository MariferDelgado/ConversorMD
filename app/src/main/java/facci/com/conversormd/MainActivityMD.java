...
public class MainActivityMD extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String tmpMonedaActual=preferencias.getString("Moneda Actual","");
        String tmpMonedaCambio=preferencias.getString("Moneda Cambio","");
        if(!tmpMonedaActual.equals("")){
            int índice = adaptador.getPosition(tmpMonedaActual);
            monedaActualSP.setSelection(índice);
        }
        if(!tmpMonedaCambio.equals("")){
            int índice = adaptador.getPosition(tmpMonedaCambio);
            monedaCambioSP.setSelection(índice);
    ...
    public void clickConvertir(View v){
        ...
        if(resultado>0){
            resultadoTV.setText(String.format("Por %5.2f %s,usted recibirá %5.2f %s",valorCambio,monedaActual,monedaCambio));
        ...
        }else
        {
            ...
            Toast.makeText(MainActivityMD.this,"las opciones elegidas no tienen un factor de conversión",Toast.LENGTH_SHORT).show();
        }
}
